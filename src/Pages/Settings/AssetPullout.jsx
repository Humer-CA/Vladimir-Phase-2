...
import AddAssetPullout from "./AddEdit/AddAssetPulltout";
...
} from "../../Redux/Query/Settings/AssetPullout";
...
// import AddAssetPullout from "../Masterlist/AddEdit/Settings/AddAssetPullout";
...
const AssetPullout = () => {
  ...
  const {
    data: assetPulloutData,
    isLoading: assetPulloutLoading,
    isSuccess: assetPulloutSuccess,
    isError: assetPulloutError,
    error: errorData,
    refetch,
  } = useGetAssetPulloutApiQuery(
    {
  ...
  const [postAssetPulloutStatusApi, { isLoading: isStatusLoading }] = usePostAssetPulloutStatusApiMutation();

  const [deleteAssetPulloutApi, { isLoading }] = useDeleteAssetPulloutApiMutation();
    ...
    dispatch(
      openConfirm({
        ...
        onConfirm: async () => {
          try {
            dispatch(onLoading());
            const result = await postAssetPulloutStatusApi({
              id: id,
    ...
    dispatch(
      openConfirm({
        ...
        onConfirm: async () => {
          try {
            dispatch(onLoading());
            let result = await deleteAssetPulloutApi({ id: id, subunit_id: id }).unwrap();
            console.log(result);
            setPage(1);
            dispatch(
              openToast({
  ...
  return (
    <>
      {assetPulloutLoading && <MasterlistSkeleton category={true} onAdd={true} />}
      {assetPulloutError && <ErrorFetching refetch={refetch} category={assetPulloutData} error={errorData} />}
      {assetPulloutData && !assetPulloutError && (
        <>
          <Box className="mcontainer__wrapper">
                ...
                <Table className="mcontainer__table" stickyHeader>
                  ...
                  <TableBody>
                    {assetPulloutData?.data?.length === 0 ? (
                      ...
                      <>
                        {assetPulloutSuccess &&
                          [...assetPulloutData?.data]?.sort(comparator(order, orderBy))?.map((data, index) => (
                            <TableRow
                              key={index}
                              sx={{
            ...
            <CustomTablePagination
              total={assetPulloutData?.total}
              success={assetPulloutSuccess}
              current_page={assetPulloutData?.current_page}
              per_page={assetPulloutData?.per_page}
