...
import AddAssetDisposal from "./AddEdit/AddAssetDisposal";
...
const AssetDisposal = () => {
  ...
  const [updateAssetDisposal, setUpdateAssetDisposal] = useState({
    status: false,
    id: null,
    requester_id: null,
    approver_id: [],
  ...
  const {
    ...
    isLoading: assetDisposalLoading,
    isSuccess: assetDisposalSuccess,
    isError: assetDisposalError,
    error: errorData,
    refetch,
  } = useGetAssetDisposalApiQuery(
    {
  ...
  const [postAssetDisposalStatusApi, { isLoading: isStatusLoading }] = usePostAssetDisposalStatusApiMutation();

  const [deleteAssetDisposalApi, { isLoading }] = useDeleteAssetDisposalApiMutation();
    ...
    dispatch(
      openConfirm({
        ...
        onConfirm: async () => {
          try {
            dispatch(onLoading());
            const result = await postAssetDisposalStatusApi({
              id: id,
  ...
  return (
    <>
      {assetDisposalLoading && <MasterlistSkeleton category={true} onAdd={true} />}
      {assetDisposalError && <ErrorFetching refetch={refetch} category={assetDisposalData} error={errorData} />}
      {assetDisposalData && !assetDisposalError && (
        <>
          <Box className="mcontainer__wrapper">
                    ...
                    {assetDisposalData?.data?.length === 0 ? (
                      ...
                      <>
                        {assetDisposalSuccess &&
                          [...assetDisposalData?.data]?.sort(comparator(order, orderBy))?.map((data, index) => (
                            <TableRow
                              key={index}
                              sx={{
            ...
            <CustomTablePagination
              total={assetDisposalData?.total}
              success={assetDisposalSuccess}
              current_page={assetDisposalData?.current_page}
              per_page={assetDisposalData?.per_page}
      ...
      >
        <AddAssetDisposal data={updateAssetDisposal} onUpdateResetHandler={onUpdateResetHandler} />
      </Dialog>
    </>
  );
};
